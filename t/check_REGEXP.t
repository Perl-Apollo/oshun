...
use Data::Checks::TestUtils 'REGEXP';
...
my    $my_scalar    :of(REGEXP) = qr{};
our   $our_scalar   :of(REGEXP) = qr{};
state $state_scalar :of(REGEXP) = qr{};

# Variables have to be initialized with something that passes the REGEXP check...
...
# Implicit undef DOESN'T pass the REGEXP check...
# (Note: can't check uninitialized our variable because that fails at compile-time)
FAIL_ON_INIT { my $uninitialized :of(REGEXP)    }    'uninitialized my scalar';
FAIL_ON_INIT { state $uninitialized :of(REGEXP) }    'uninitialized state scalar';

# Other explicit initializer values also don't pass the REGEXP check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::Parser::pp($bad_value);
    FAIL_ON_INIT { my $uninitialized :of(REGEXP)    = $bad_value }  "   my scalar = $bad_value_str";
    FAIL_ON_INIT { our $uninitialized :of(REGEXP)   = $bad_value }  "  our scalar = $bad_value_str";
    FAIL_ON_INIT { state $uninitialized :of(REGEXP) = $bad_value }  "state scalar = $bad_value_str";
}

# Assignments must likewise pass the REGEXP check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::Parser::pp($good_value);
...
# Test subroutines: parameters, internal variables, return values...
      ...
      sub   old_sub :returns(REGEXP)                     { my $x :of(REGEXP) = shift; return $x }
      sub   new_sub :returns(REGEXP)  ($param :of(REGEXP))  { return $param }
my    sub    my_sub :returns(REGEXP)  ($param :of(REGEXP))  { return $param }
state sub state_sub :returns(REGEXP)  ($param :of(REGEXP))  { return $param }

      sub   old_ret_sub : returns(REGEXP)            { return shift  }
      sub   new_ret_sub : returns(REGEXP)  ($param)  { return $param }
my    sub    my_ret_sub : returns(REGEXP)  ($param)  { return $param }
state sub state_ret_sub : returns(REGEXP)  ($param)  { return $param }

# With values that should pass the REGEXP check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::Parser::pp($good_value);
...
# With values that SHOULDN'T pass the REGEXP check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::Parser::pp($bad_value);
