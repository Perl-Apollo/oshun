...
use Data::Checks::TestUtils 'ISA[Class::Base]';
...
my    $my_scalar    :of(ISA[Class::Base]) = bless {}, 'Class::Base';
our   $our_scalar   :of(ISA[Class::Base]) = bless {}, 'Class::Base';
state $state_scalar :of(ISA[Class::Base]) = bless {}, 'Class::Base';

# Variables have to be initialized with something that passes the ISA[Class::Base] check...
...
# Implicit undef DOESN'T pass the ISA[Class::Base] check...
# (Note: can't check uninitialized our variable because that fails at compile-time)
FAIL_ON_INIT { my $uninitialized :of(ISA[Class::Base])    }    'uninitialized my scalar';
FAIL_ON_INIT { state $uninitialized :of(ISA[Class::Base]) }    'uninitialized state scalar';

# Other explicit initializer values also don't pass the ISA[Class::Base] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::pp($bad_value);
    FAIL_ON_INIT { my $uninitialized :of(ISA[Class::Base])    = $bad_value }  "   my scalar = $bad_value_str";
    FAIL_ON_INIT { our $uninitialized :of(ISA[Class::Base])   = $bad_value }  "  our scalar = $bad_value_str";
    FAIL_ON_INIT { state $uninitialized :of(ISA[Class::Base]) = $bad_value }  "state scalar = $bad_value_str";
}

# Assignments must likewise pass the ISA[Class::Base] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::pp($good_value);
...
# Test subroutines: parameters, internal variables, return values...
      ...
      sub   old_sub :returns(ISA[Class::Base])                     { my $x :of(ISA[Class::Base]) = shift; return $x }
      sub   new_sub :returns(ISA[Class::Base])  ($param :of(ISA[Class::Base]))  { return $param }
my    sub    my_sub :returns(ISA[Class::Base])  ($param :of(ISA[Class::Base]))  { return $param }
state sub state_sub :returns(ISA[Class::Base])  ($param :of(ISA[Class::Base]))  { return $param }

      sub   old_ret_sub : returns(ISA[Class::Base])            { return shift  }
      sub   new_ret_sub : returns(ISA[Class::Base])  ($param)  { return $param }
my    sub    my_ret_sub : returns(ISA[Class::Base])  ($param)  { return $param }
state sub state_ret_sub : returns(ISA[Class::Base])  ($param)  { return $param }

# With values that should pass the ISA[Class::Base] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::pp($good_value);
...
# With values that SHOULDN'T pass the ISA[Class::Base] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::pp($bad_value);
