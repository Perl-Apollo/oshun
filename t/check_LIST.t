...
use Data::Checks::TestUtils 'LIST';
...
# Can't declare variables with a LIST check...
ok !eval q{ my $uninitialized :of(LIST)    }  =>  'uninitialized my scalar';
ok $@ =~ /\QCan't specify :of(LIST) on a scalar variable\E/ => '  \___with correct exception';
ok !eval q{ our $uninitialized :of(LIST|ARRAY)   }  =>  'uninitialized our scalar';
ok $@ =~ /\QCan't specify :of(LIST|ARRAY) on a scalar variable\E/ => '  \___with correct exception';
ok !eval q{ state $uninitialized :of(!LIST) }  =>  'uninitialized state scalar';
ok $@ =~ /\QCan't specify :of(!LIST) on a scalar variable\E/ => '  \___with correct exception';


# Test subroutines...

      sub   old_ret_sub : returns(LIST)            { return shift  }
      sub   new_ret_sub : returns(LIST)  ($param)  { return () }
my    sub    my_ret_sub : returns(LIST)  ($param)  { return ($param, $param) }
state sub state_ret_sub : returns(LIST)  ($param)  { return ($param, $param, $param) }

# With values that should pass the LIST check...
