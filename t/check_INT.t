...
sub BAD_VALUES {
    100000000000.1, -10.1, -1.1, 0.1, 1.1, 7.1, 99.9,
      q{}, q{string},
      ~0 + 1,    # --> first value greater than MAXINT
      v1, v1.2, v1.2.3,
...
my $my_scalar : of(INT) = 0;
our $our_scalar : of(INT) = 0;
state $state_scalar : of(INT) = 0;
...
# Implicit undef DOESN'T pass the INT check...
# (Note: can't check uninitialized our variable because that fails at compile-time)
FAIL_ON_INIT { my $uninitialized : of(INT) } 'uninitialized my scalar';
FAIL_ON_INIT { state $uninitialized : of(INT) } 'uninitialized state scalar';

# Other explicit initializer values also don't pass the INT check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::Parser::pp($bad_value);
    FAIL_ON_INIT { my $uninitialized : of(INT)    = $bad_value } "   my scalar = $bad_value_str";
    FAIL_ON_INIT { our $uninitialized : of(INT)   = $bad_value } "  our scalar = $bad_value_str";
    FAIL_ON_INIT { state $uninitialized : of(INT) = $bad_value } "state scalar = $bad_value_str";
}

# Assignments must likewise pass the INT check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::Parser::pp($good_value);
...
sub old_sub : returns(INT) { my $x : of(INT) = shift; return $x }
sub new_sub : returns(INT) ( $param : of(INT) ) { return $param }
my sub my_sub : returns(INT) ( $param : of(INT) ) { return $param }
state sub state_sub : returns(INT) ( $param : of(INT) ) { return $param }

sub old_ret_sub : returns(INT) { return shift }
sub new_ret_sub : returns(INT) ($param) { return $param }
my sub my_ret_sub : returns(INT) ($param) { return $param }
state sub state_ret_sub : returns(INT) ($param) { return $param }
