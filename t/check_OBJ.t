...
my    $my_scalar    :of(OBJ) = bless {}, 'Null';
our   $our_scalar   :of(OBJ) = bless {}, 'Null';
state $state_scalar :of(OBJ) = bless {}, 'Null';

# Variables have to be initialized with something that passes the OBJ check...
...
# Implicit undef DOESN'T pass the OBJ check...
# (Note: can't check uninitialized our variable because that fails at compile-time)
FAIL_ON_INIT { my $uninitialized :of(OBJ)    }    'uninitialized my scalar';
FAIL_ON_INIT { state $uninitialized :of(OBJ) }    'uninitialized state scalar';

# Other explicit initializer values also don't pass the OBJ check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::pp($bad_value);
    FAIL_ON_INIT { my $uninitialized :of(OBJ)    = $bad_value }  "   my scalar = $bad_value_str";
    FAIL_ON_INIT { our $uninitialized :of(OBJ)   = $bad_value }  "  our scalar = $bad_value_str";
    FAIL_ON_INIT { state $uninitialized :of(OBJ) = $bad_value }  "state scalar = $bad_value_str";
}

# Assignments must likewise pass the OBJ check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::pp($good_value);
...
# Test subroutines: parameters, internal variables, return values...
      ...
      sub   old_sub :returns(OBJ)                     { my $x :of(OBJ) = shift; return $x }
      sub   new_sub :returns(OBJ)  ($param :of(OBJ))  { return $param }
my    sub    my_sub :returns(OBJ)  ($param :of(OBJ))  { return $param }
state sub state_sub :returns(OBJ)  ($param :of(OBJ))  { return $param }

      sub   old_ret_sub : returns(OBJ)            { return shift  }
      sub   new_ret_sub : returns(OBJ)  ($param)  { return $param }
my    sub    my_ret_sub : returns(OBJ)  ($param)  { return $param }
state sub state_ret_sub : returns(OBJ)  ($param)  { return $param }

# With values that should pass the OBJ check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::pp($good_value);
...
# With values that SHOULDN'T pass the OBJ check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::pp($bad_value);
