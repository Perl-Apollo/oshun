...

{
    ...
    for my $good_value (GOOD_VALUES) {
        ...
        WARN_ON_RETURN {;   old_ret_sub( $good_value ) }   "  old_sub( $good_value_str )";
        WARN_ON_RETURN {;   new_ret_sub( $good_value ) }   "  new_sub( $good_value_str )";
        WARN_ON_RETURN {;    my_ret_sub( $good_value ) }   "   my_sub( $good_value_str )";
        WARN_ON_RETURN {; state_ret_sub( $good_value ) }   "state_sub( $good_value_str )";
    }
    ...
    for my $bad_value (BAD_VALUES) {
        ...
        WARN_ON_RETURN { scalar   new_sub( $bad_value ) }       "  new_sub( $bad_value_str )";
        WARN_ON_RETURN { scalar    my_sub( $bad_value ) }       "   my_sub( $bad_value_str )";
        WARN_ON_RETURN { scalar state_sub( $bad_value ) }       "state_sub( $bad_value_str )";

        # Can't return invalid values in any context...
        WARN_ON_RETURN { scalar   old_ret_sub( $bad_value ) }   "  old_sub( $bad_value_str )";
        WARN_ON_RETURN { scalar   new_ret_sub( $bad_value ) }   "  new_sub( $bad_value_str )";
        WARN_ON_RETURN { scalar    my_ret_sub( $bad_value ) }   "   my_sub( $bad_value_str )";
        WARN_ON_RETURN { scalar state_ret_sub( $bad_value ) }   "state_sub( $bad_value_str )";
        WARN_ON_RETURN { () =     old_ret_sub( $bad_value ) }   "  old_sub( $bad_value_str )";
        WARN_ON_RETURN { () =     new_ret_sub( $bad_value ) }   "  new_sub( $bad_value_str )";
        WARN_ON_RETURN { () =      my_ret_sub( $bad_value ) }   "   my_sub( $bad_value_str )";
        WARN_ON_RETURN { () =   state_ret_sub( $bad_value ) }   "state_sub( $bad_value_str )";
        WARN_ON_RETURN {;         old_ret_sub( $bad_value ) }   "  old_sub( $bad_value_str )";
        WARN_ON_RETURN {;         new_ret_sub( $bad_value ) }   "  new_sub( $bad_value_str )";
        WARN_ON_RETURN {;          my_ret_sub( $bad_value ) }   "   my_sub( $bad_value_str )";
        WARN_ON_RETURN {;       state_ret_sub( $bad_value ) }   "state_sub( $bad_value_str )";
...
}
{
    ...
    WARN_ON_RETURN {;         new_ret_sub( undef ) }   "  new_sub( undef )";
    ...
    FAIL_ON_RETURN {;         new_ret_sub( undef ) }   "  new_sub( undef )";
