...
use Data::Checks::TestUtils 'HASH[ARRAY[UINT|CODE]]';
...
my $my_scalar : of(HASH[ARRAY[UINT|CODE]]) = do { my %x; \%x };
our $our_scalar : of(HASH[ARRAY[UINT|CODE]]) = do { my %x; \%x };
state $state_scalar : of(HASH[ARRAY[UINT|CODE]]) = do { my %x; \%x };

# Variables have to be initialized with something that passes the HASH[ARRAY[UINT|CODE]] check...
...
# Implicit undef DOESN'T pass the HASH[ARRAY[UINT|CODE]] check...
# (Note: can't check uninitialized our variable because that fails at compile-time)
FAIL_ON_INIT { my $uninitialized : of(HASH[ARRAY[UINT|CODE]]) } 'uninitialized my scalar';
FAIL_ON_INIT { state $uninitialized : of(HASH[ARRAY[UINT|CODE]]) } 'uninitialized state scalar';

# Other explicit initializer values also don't pass the HASH[ARRAY[UINT|CODE]] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::Parser::pp($bad_value);
    FAIL_ON_INIT { my $uninitialized : of(HASH[ARRAY[UINT|CODE]])    = $bad_value } "   my scalar = $bad_value_str";
    FAIL_ON_INIT { our $uninitialized : of(HASH[ARRAY[UINT|CODE]])   = $bad_value } "  our scalar = $bad_value_str";
    FAIL_ON_INIT { state $uninitialized : of(HASH[ARRAY[UINT|CODE]]) = $bad_value } "state scalar = $bad_value_str";
}

# Assignments must likewise pass the HASH[ARRAY[UINT|CODE]] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::Parser::pp($good_value);
...
sub old_sub : returns(HASH[ARRAY[UINT|CODE]]) { my $x : of(HASH[ARRAY[UINT|CODE]]) = shift; return $x }
sub new_sub : returns(HASH[ARRAY[UINT|CODE]]) ( $param : of(HASH[ARRAY[UINT|CODE]]) ) { return $param }
my sub my_sub : returns(HASH[ARRAY[UINT|CODE]]) ( $param : of(HASH[ARRAY[UINT|CODE]]) ) { return $param }
state sub state_sub : returns(HASH[ARRAY[UINT|CODE]]) ( $param : of(HASH[ARRAY[UINT|CODE]]) ) { return $param }

sub old_ret_sub : returns(HASH[ARRAY[UINT|CODE]]) { return shift }
sub new_ret_sub : returns(HASH[ARRAY[UINT|CODE]]) ($param) { return $param }
my sub my_ret_sub : returns(HASH[ARRAY[UINT|CODE]]) ($param) { return $param }
state sub state_ret_sub : returns(HASH[ARRAY[UINT|CODE]]) ($param) { return $param }

# With values that should pass the HASH[ARRAY[UINT|CODE]] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::Parser::pp($good_value);
...
# With values that SHOULDN'T pass the HASH[ARRAY[UINT|CODE]] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::Parser::pp($bad_value);
