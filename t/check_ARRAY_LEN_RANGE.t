...
use Data::Checks::TestUtils 'ARRAY[0..9 => INT]';
...
my $my_scalar :of(ARRAY[0..9 => INT]) = do { my @x; \@x };
state $state_scalar :of(ARRAY[0..9 => INT]) = do { my @x; \@x };

# Variables have to be initialized with something that passes the ARRAY[0..9 => INT] check...
...
# Implicit undef DOESN'T pass the ARRAY[0..9 => INT] check...
# (Note: can't check uninitialized our variable because that fails at compile-time)
FAIL_ON_INIT { my $uninitialized :of(ARRAY[0..9 => INT]) } 'uninitialized my scalar';
FAIL_ON_INIT { state $uninitialized :of(ARRAY[0..9 => INT]) } 'uninitialized state scalar';

# Other explicit initializer values also don't pass the ARRAY[0..9 => INT] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::Parser::pp($bad_value);
    FAIL_ON_INIT { my $uninitialized :of(ARRAY[0..9 => INT]) = $bad_value } "   my scalar = $bad_value_str";
    FAIL_ON_INIT { state $uninitialized :of(ARRAY[0..9 => INT]) = $bad_value } "state scalar = $bad_value_str";
}

# Assignments must likewise pass the ARRAY[0..9 => INT] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::Parser::pp($good_value);
...
sub old_sub :returns(ARRAY[0..9 => INT]) { my $x :of(ARRAY[0..9 => INT]) = shift; return $x }
sub new_sub :returns(ARRAY[0..9 => INT]) ( $param :of(ARRAY[0..9 => INT]) ) { return $param }
my sub my_sub :returns(ARRAY[0..9 => INT]) ( $param :of(ARRAY[0..9 => INT]) ) { return $param }
state sub state_sub :returns(ARRAY[0..9 => INT]) ( $param :of(ARRAY[0..9 => INT]) ) { return $param }

sub old_ret_sub :returns(ARRAY[0..9 => INT]) { return shift }
sub new_ret_sub :returns(ARRAY[0..9 => INT]) ($param) { return $param }
my sub my_ret_sub :returns(ARRAY[0..9 => INT]) ($param) { return $param }
state sub state_ret_sub :returns(ARRAY[0..9 => INT]) ($param) { return $param }

# With values that should pass the ARRAY[0..9 => INT] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::Parser::pp($good_value);
...
# With values that SHOULDN'T pass the ARRAY[0..9 => INT] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::Parser::pp($bad_value);
