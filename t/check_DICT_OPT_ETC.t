...
use Data::Checks::TestUtils q{DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC]};
...
sub GOOD_VALUES {
    ...
    {            '*u*' => 0, 99 => '',      etc => {a=>1, b=>2}                   },
    {            '*u*' => 0, 99 => '',      etc => {a=>1, b=>2}, other => 'extra' },
}
...
my    $my_scalar    :of(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC])
    = {num=>1.1, '*u*'=>2, 99=>'three', etc=>{}};
our   $our_scalar   :of(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC])
    = {num=>1.1, '*u*'=>2, 99=>'three', etc=>{}};
state $state_scalar :of(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC])
    = {num=>1.1, '*u*'=>2, 99=>'three', etc=>{}};
...
# Test subroutines: parameters, internal variables, return values...
      ...
      sub   old_sub :returns(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC])                     { my $x :of(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC]) = shift; return $x }
      sub   new_sub :returns(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC])  ($param :of(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC]))  { return $param }
my    sub    my_sub :returns(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC])  ($param :of(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC]))  { return $param }
state sub state_sub :returns(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC])  ($param :of(DICT[OPT[ num => NUM ], qq{*u*} => UINT, 99 => STR, 'etc' => HASH, ETC]))  { return $param }
