...
use Data::Checks::TestUtils q{DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH]};
...
my    $my_scalar    :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])
    = {num=>1.1, '*u*'=>2, 99=>'three', etc=>{}};
our   $our_scalar   :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])
    = {num=>1.1, '*u*'=>2, 99=>'three', etc=>{}};
state $state_scalar :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])
    = {num=>1.1, '*u*'=>2, 99=>'three', etc=>{}};

# Variables have to be initialized with something that passes the DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH] check...
...
# Implicit undef DOESN'T pass the DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH] check...
# (Note: can't check uninitialized our variable because that fails at compile-time)
FAIL_ON_INIT { my $uninitialized :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])    }    'uninitialized my scalar';
FAIL_ON_INIT { state $uninitialized :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH]) }    'uninitialized state scalar';

# Other explicit initializer values also don't pass the DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::Parser::pp($bad_value);
    FAIL_ON_INIT { my $uninitialized :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])    = $bad_value }  "   my scalar = $bad_value_str";
    FAIL_ON_INIT { our $uninitialized :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])   = $bad_value }  "  our scalar = $bad_value_str";
    FAIL_ON_INIT { state $uninitialized :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH]) = $bad_value }  "state scalar = $bad_value_str";
}

# Assignments must likewise pass the DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::Parser::pp($good_value);
...
# Test subroutines: parameters, internal variables, return values...
      ...
      sub   old_sub :returns(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])                     { my $x :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH]) = shift; return $x }
      sub   new_sub :returns(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])  ($param :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH]))  { return $param }
my    sub    my_sub :returns(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])  ($param :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH]))  { return $param }
state sub state_sub :returns(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])  ($param :of(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH]))  { return $param }

      sub   old_ret_sub : returns(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])            { return shift  }
      sub   new_ret_sub : returns(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])  ($param)  { return $param }
my    sub    my_ret_sub : returns(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])  ($param)  { return $param }
state sub state_ret_sub : returns(DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH])  ($param)  { return $param }

# With values that should pass the DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::Parser::pp($good_value);
...
# With values that SHOULDN'T pass the DICT['num' => NUM, qq{*u*} => UINT, 99 => STR, 'etc' => HASH] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::Parser::pp($bad_value);
