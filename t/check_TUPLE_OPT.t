...
use Data::Checks::TestUtils 'TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]';
...
my    $my_scalar    :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]) = [1.1, 2, 'three', {}];
our   $our_scalar   :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]) = [1.1, 2, 'three', {}];
state $state_scalar :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]) = [1.1, 2, 'three', {}];

# Variables have to be initialized with something that passes the TUPLE[NUM, UINT, OPT[STR], OPT[HASH]] check...
...
# Implicit undef DOESN'T pass the TUPLE[NUM, UINT, OPT[STR], OPT[HASH]] check...
# (Note: can't check uninitialized our variable because that fails at compile-time)
FAIL_ON_INIT { my $uninitialized :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])    }    'uninitialized my scalar';
FAIL_ON_INIT { state $uninitialized :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]) }    'uninitialized state scalar';

# Other explicit initializer values also don't pass the TUPLE[NUM, UINT, OPT[STR], OPT[HASH]] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::pp($bad_value);
    FAIL_ON_INIT { my $uninitialized :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])    = $bad_value }  "   my scalar = $bad_value_str";
    FAIL_ON_INIT { our $uninitialized :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])   = $bad_value }  "  our scalar = $bad_value_str";
    FAIL_ON_INIT { state $uninitialized :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]) = $bad_value }  "state scalar = $bad_value_str";
}

# Assignments must likewise pass the TUPLE[NUM, UINT, OPT[STR], OPT[HASH]] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::pp($good_value);
...
# Test subroutines: parameters, internal variables, return values...
      ...
      sub   old_sub :returns(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])                     { my $x :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]) = shift; return $x }
      sub   new_sub :returns(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])  ($param :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]))  { return $param }
my    sub    my_sub :returns(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])  ($param :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]))  { return $param }
state sub state_sub :returns(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])  ($param :of(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]]))  { return $param }

      sub   old_ret_sub : returns(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])            { return shift  }
      sub   new_ret_sub : returns(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])  ($param)  { return $param }
my    sub    my_ret_sub : returns(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])  ($param)  { return $param }
state sub state_ret_sub : returns(TUPLE[NUM, UINT, OPT[STR], OPT[HASH]])  ($param)  { return $param }

# With values that should pass the TUPLE[NUM, UINT, OPT[STR], OPT[HASH]] check...
for my $good_value (GOOD_VALUES) {
    my $good_value_str = Data::Checks::pp($good_value);
...
# With values that SHOULDN'T pass the TUPLE[NUM, UINT, OPT[STR], OPT[HASH]] check...
for my $bad_value (BAD_VALUES) {
    my $bad_value_str = Data::Checks::pp($bad_value);
