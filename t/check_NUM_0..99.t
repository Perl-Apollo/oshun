...
use Data::Checks::TestUtils 'NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]';
...
my    $my_scalar    :of(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]) = 0;
our   $our_scalar   :of(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]) = 0;
state $state_scalar :of(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]) = 0;
...
# Implicit undef DOESN'T pass the NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT] check...
# (Note: can't check uninitialized our variable because that fails at compile-time)
FAIL_ON_INIT { my $uninitialized :of(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT])    }    'uninitialized my scalar';
FAIL_ON_INIT { state $uninitialized :of(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]) }    'uninitialized state scalar';
...
# Test subroutines: parameters, internal variables, return values...
      ...
      sub   old_sub :returns(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]) { my $x :of(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]) = shift; return $x }
      sub   new_sub :returns(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT])  ($param :of(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]))  { return $param }
my    sub    my_sub :returns(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT])  ($param :of(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]))  { return $param }
state sub state_sub :returns(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT])  ($param :of(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT]))  { return $param }

      sub   old_ret_sub : returns(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT])            { return shift  }
      sub   new_ret_sub : returns(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT])  ($param)  { return $param }
my    sub    my_ret_sub : returns(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT])  ($param)  { return $param }
state sub state_ret_sub : returns(NUM[-1..<-0.1, 0.1<..1, /^-[23]$/, INT])  ($param)  { return $param }
